...
void CServerSecurityConfig::addPermission(const void *json_handle, CServerSecCfg &cfg,
                              int32_t level, bool is_group, std::string &err_msg)
{
                                ...
                                const char *json_str, std::string &err_msg)
{
    ...
    if (cfg_root != NULL)
    {
        if (cJSON_IsObject(cfg_root))
        {
                            ...
                            if (cJSON_IsObject(sec_level))
                            {
                                ...
                                if (item_level)
                                {
                                    if (cJSON_IsNumber(item_level))
                                    {
                                        ...
                                        if (item_gid)
                                        {
                                            addPermission(item_gid, perm.mGid, level, true, err_msg);
                                        }
                                        auto item_uid = cJSON_GetObjectItem(sec_level, "uid");
                                        if (item_uid)
                                        {
                                            addPermission(item_uid, perm.mUid, level, false, err_msg);
                                        }
                                    }
                                    else
                                    {
            ...
            auto network = cJSON_GetObjectItem(cfg_root, "network");
            if (network != NULL)
            {
                if (cJSON_IsArray(network))
                {
                    auto &network_list = mServiceNetworkList[svc_name];
                    for (int i = 0; i < cJSON_GetArraySize(network); ++i)
                    {
                        auto url = cJSON_GetArrayItem(network, i);
                        if (cJSON_IsString(url))
                        {
                            network_list.resize(network_list.size() + 1);
                            auto &addr = network_list.back();
                            if (!CBaseSocketFactory::parseUrl(url->valuestring, addr))
                            {
