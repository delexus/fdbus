...
#define FDB_MSG_TX_NO_QUEUE     (1 << 2)
...
protected:
    void run()
    {
        mSession->terminateMessage(mMsgSn, FDB_ST_TIMEOUT, "Message is destroyed due to timeout.");
    }
private:
...
CFdbMessage::CFdbMessage(FdbMsgCode_t code, EFdbQOS qos)
...
CFdbMessage::CFdbMessage(FdbMsgCode_t code, CFdbMessage *msg, const char *topic)
...
CFdbMessage::CFdbMessage(CFdbMessageHeader &head
    ...
    , mLifeTime(0)
{
    if (head.has_broadcast_topic())
    {
        mTopic = head.broadcast_topic().c_str();
    }
    ...
    const static tHandleFn mHandle[] =
        {
            ...
            &CFdbMessage::doPublish,    //FDB_MT_PUBLISH = 10
            &CFdbMessage::doRequest,    //FDB_MT_SET_EVENT = 11
        };
                         ...
                         , EFdbMessageType type)
{
    if (!mContext)
    {
        setStatusMsg(FDB_ST_UNABLE_TO_SEND, "no context");
        return false;
    }
    mType = type;
    ...
    {
        mFlag &= ~MSG_FLAG_REPLIED;
        setStatusMsg(FDB_ST_UNABLE_TO_SEND, "Fail to send job to context");
        return false;
    }
    return true;
}
                      ...
                      , const char *log_data)
{
    ...
    if (!fdb_msg->mContext)
    {
        fdb_msg->setStatusMsg(FDB_ST_UNABLE_TO_SEND, "no context");
        return false;
    }
    if (fdb_msg->mFlag & MSG_FLAG_NOREPLY_EXPECTED)
    {
...
bool CFdbMessage::set()
{
    CBaseJob::Ptr msg_ref(this);
    mType = FDB_MT_SET_EVENT;
...
bool CFdbMessage::get(CBaseJob::Ptr &msg_ref, int32_t timeout)
{
    ...
    if (msg)
    {
        ...
        return msg->submit(msg_ref, FDB_MSG_TX_SYNC, timeout);
    }
    return false;
}
...
bool CFdbMessage::broadcast(FdbMsgCode_t code
                           ...
                           , const char *topic)
{
    auto msg = new CFdbMessage(code, this, topic);
...
bool CFdbMessage::subscribeNoQueue(int32_t timeout)
{
    CBaseJob::Ptr msg_ref(this);
    return subscribe(msg_ref, FDB_MSG_TX_NO_QUEUE, FDB_CODE_SUBSCRIBE, timeout);
}
...
bool CFdbMessage::buildHeader(CBaseSession *session)
{
    ...
    msg_hdr.qos(session->qos());

    encodeDebugInfo(msg_hdr);

    auto topic = mTopic.c_str();
    if (topic[0] != '\0')
    {
        msg_hdr.set_broadcast_topic(topic);
    }
...
void CFdbMessage::doRequest(Ptr &ref)
{
    ...
    if (session)
    {
        ...
        {
            success = session->sendMessageRef(ref);
            if (success)
            {
                if (mTimer)
                {
    ...
    if (!success)
    {
        ...
        {
            setStatusMsg(FDB_ST_INVALID_ID, reason);
        }
        else
        {
            onAsyncError(ref, FDB_ST_INVALID_ID, reason);
        }
    }
}

void CFdbMessage::doPublish(Ptr &ref)
{
    bool success = true;
    const char *reason = "";
    ...
    if (endpoint)
    {
        if (fdbValidFdbId(mSid))
        {
            ...
            if (session)
            {
                ...
                if (object)
                {
                    // publish to specified session of object
                    if (!object->publish(this, session))
                    {
                        success = false;
        ...
        {
            auto object = endpoint->getObject(this, false);
            if (object)
            {
                // publish to event cache
                object->publish(this, endpoint->preferredPeer(mQOS));
            }
            else
            {
                success = false;
...
void CFdbMessage::sendStatus(CBaseSession *session, int32_t error_code, const char *description)
{
    if (!(mFlag & MSG_FLAG_NOREPLY_EXPECTED))
    {
...
void CFdbMessage::autoReply(CBaseSession *session
...
void CFdbMessage::doAutoReply(CBaseJob::Ptr &msg_ref, int32_t error_code, const char *description)
{
    if (needReply())
    {
        setStatusMsg(error_code, description, FDB_MT_STATUS);
        setCallable(std::bind(&CFdbMessage::dispatchMsg, this, _1));
        mContext->sendAsync(msg_ref);
    }
}
...
void CFdbMessage::update(CFdbMessageHeader &head, CFdbMsgPrefix &prefix)
{
...
CFdbSession *CFdbMessage::getSession()
{
    ...
    if (endpoint)
    {
        return fdbValidFdbId(mSid) ? endpoint->getSession(mSid) :
                                     endpoint->preferredPeer(mQOS);
    }
    return 0;
}
...
void CFdbMessage::checkLogEnabled(const CFdbBaseObject *object)
{
    if (!object)
    {
        if (!(mFlag & MSG_FLAG_ENABLE_LOG))
        {
            ...
            if (logger && logger->checkLogEnabled(mType, mStringData.c_str(), 0))
            {
...
void CFdbMessage::encodeDebugInfo(CFdbMessageHeader &msg_hdr)
{
    if (!mTimeStamp)
    {
        return;
...
void CFdbMessage::decodeDebugInfo(CFdbMessageHeader &msg_hdr)
{
    if (!mTimeStamp)
    {
        return;
...
bool CFdbMessage::kickDog(CBaseJob::Ptr &msg_ref, CBaseWorker *worker, tJobCallable fn)
{
    if (!worker || !fn)
    {
        return false;
...
void CFdbMessage::callbackWithAutoReply(CBaseJob::Ptr &msg_ref, tJobCallable &func)
{
    if (func)
    {
    ...
    autoReply(msg_ref, FDB_ST_AUTO_REPLY_OK, "Automatically reply to request.");
}
}
}
