...
protected:
    void onReply(CBaseJob::Ptr &msg_ref)
    {
        ...
        switch (msg->code())
        {
            case REQ_QUERY_HOST_LOCAL:
            {
                FdbMsgHostAddressList host_list;
                CFdbParcelableParser parser(host_list);
                if (!msg->deserialize(parser))
                {
                    LOG_E("CNameServerProxy: unable to decode FdbMsgHostAddressList.\n");
                }
                else
                {
    ...
    void onOnline(const CFdbOnlineInfo &info)
    {
        if (ls_follow)
        {
            ...
            addNotifyItem(subscribe_list, NTF_HOST_ONLINE_LOCAL);
            subscribe(subscribe_list, 0, info.mQOS);
        }
        else
        {
            invoke(REQ_QUERY_HOST_LOCAL, 0, info.mQOS);
        }
    }
...
private:
    ...
    void printHosts(FdbMsgHostAddressList &host_list, bool monitor)
    {
        auto &addr_list = host_list.address_list();
        for (auto it = addr_list.vpool().begin(); it != addr_list.vpool().end(); ++it)
        {
            auto &addr = *it;
            bool is_offline = addr.address_list().pool().empty();
            if (is_offline)
            {
