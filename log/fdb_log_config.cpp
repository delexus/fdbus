...
void fdb_print_configuration()
{
    ...
    std::cout << "static content:       " << (gFdbLogConfig.fdb_static_content ? "true" : "false") << std::endl;
...
void fdb_fill_logger_config(FdbMsgLogConfig &config, bool global_enabled)
{
    ...
    config.set_static_content(gFdbLogConfig.fdb_static_content);
}

void fdb_fill_trace_config(FdbTraceConfig &config, bool global_enabled)
{
    config.set_global_enable(global_enabled);
...
void fdb_dump_logger_config(FdbMsgLogConfig &config)
{
    ...
    gFdbLogConfig.fdb_static_content = config.static_content();
}

void fdb_dump_trace_config(FdbTraceConfig &config)
{
...
bool fdb_update_logger_config(FdbMsgLogConfig &config, bool global_enabled)
{
    bool updated = false;
    ...
    if (gFdbLogConfig.fdb_static_content_configured)
    {
        config.set_static_content(gFdbLogConfig.fdb_static_content);
        updated = true;
    }
...
bool fdb_update_trace_config(FdbTraceConfig &config, bool global_enabled)
{
    bool updated = false;
...
void printHelp(bool is_viewer)
{
    ...
    if (is_viewer)
    {
        ...
        std::cout << "       logviewer -k" << std::endl;
        std::cout << "           sync file cache in log server or" << std::endl;
    ...
    if (!is_viewer)
    {
        ...
        std::cout << "    -C: compress logs to zip file" << std::endl;
        std::cout << "    -y: exportable level" << std::endl;
    }
...
bool initLogConfig(int argc, char **argv, bool is_viewer)
{
    ...
    fdb_init_conifg(fdb_static_content, false);
    ...
    gFdbLogConfig.fdb_log_compress = 0;
    gFdbLogConfig.fdb_exportable_level = -1;
    ...
    const struct fdb_option core_options[] = {
        ...
        { FDB_OPTION_BOOLEAN, "static_content", 'a', &gFdbLogConfig.fdb_static_content },

        // for logsvc
        { FDB_OPTION_BOOLEAN, "output", 'o', &gFdbLogConfig.fdb_disable_std_output },
        { FDB_OPTION_BOOLEAN, "output", 'C', &gFdbLogConfig.fdb_log_compress },
        { FDB_OPTION_INTEGER, "exportable level", 'y', &gFdbLogConfig.fdb_exportable_level },
        ...
        { FDB_OPTION_BOOLEAN, "sync", 'k', &gFdbLogConfig.fdb_sync },
