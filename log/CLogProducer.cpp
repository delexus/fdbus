...
namespace ipc {
namespace fdbus {
bool CLogProducer::mAppendLineEnd = false;
bool CLogProducer::mStaticContent = false;
...
void CLogProducer::sendCheckpoint(const char *check_point)
{
    ...
    logFDBus(&msg, 0, this, FDB_LOG_QOS);
}

void CLogProducer::onOnline(const CFdbOnlineInfo &info)
{
    CFdbMsgSubscribeList subscribe_list;
    addNotifyItem(subscribe_list, NTF_LOGGER_CONFIG);
    addNotifyItem(subscribe_list, NTF_TRACE_CONFIG);
...
void CLogProducer::onOffline(const CFdbOnlineInfo &info)
{
}
    ...
    if (!mLogEndpointWhiteList.empty() || !mLogBusnameWhiteList.empty())
    {
        ...
        if (!mLogEndpointWhiteList.empty())
        {
            ...
            else
            {
                ...
                auto it_receiver = mLogEndpointWhiteList.find(receiver_name);
                bool exclude = ((it_sender == mLogEndpointWhiteList.end()) &&
                                (it_receiver == mLogEndpointWhiteList.end()));
                if (mReverseEndpoints ^ exclude)
                {
                    return false;
                }
                            ...
                            EFdbQOS session_qos, CFdbRawMsgBuilder &builder)
{
    ...
    uint8_t option = mAppendLineEnd ? FDB_LOG_APPEND_LINE_END : 0;
    option |= mStaticContent ? FDB_LOG_STATIC_CONTENT : 0;
    if (!msg->mStringData.empty())
    {
        option |= FDB_LOG_IS_STRING;
        builder.serializer() << option << msg->mStringData;
    }
    else
    {
        builder.serializer() << option;
...
void CLogProducer::logFDBus(CFdbMessage *msg, const char *receiver_name,
                            CBaseEndpoint *endpoint, EFdbQOS session_qos)
{
    CFdbRawMsgBuilder builder;
    logFDBus(msg, receiver_name, endpoint, session_qos, builder);
    sendLog(REQ_FDBUS_LOG, builder);
}
...
void CLogProducer::printTrace(EFdbLogLevel log_level, const char *tag, const char *info)
{
    ...
    trace_info.mOptions = mAppendLineEnd ? FDB_LOG_APPEND_LINE_END : 0;
    trace_info.mOptions |= mStaticContent ? FDB_LOG_STATIC_CONTENT : 0;
...
void CLogProducer::populateWhiteList(const CFdbParcelableArray<std::string> &in_topic
                                   , tFilterTbl &white_list)
{
    white_list.clear();
    for (auto it = in_topic.pool().begin(); it != in_topic.pool().end(); ++it)
    {
...
bool CLogProducer::sendLog(FdbMsgCode_t code, const uint8_t *buffer, int32_t size)
{
    if (context()->isSelf())
    {
        if (connected())
        {
            CBaseMessage msg(code, this, FDB_INVALID_ID, FDB_LOG_QOS);
            msg.expectReply(false);
            if (!msg.serialize(buffer, size))
            {
                return false;
            }
            auto session = mEndpoint->preferredPeer(FDB_LOG_QOS);
            if (session)
            {
                msg.enableLog(false);
    ...
    {
        auto msg = new CBaseMessage(code, this, FDB_INVALID_ID, FDB_LOG_QOS);
        if (!msg->serialize(buffer, size))
        {
            delete msg;
            return false;
